...
MTU поддерживается на уровне интерфейсов и TCP сокета через обмен MSS при установке соединения.
...
2. Декларативно - демон `netd` при старте применяет сохраненное состояние, а изменения делаются командами. За работу этого механизма отвечает подсистема `nd.lib`.
