...
server:
  - listen: 80
    ...
    },
    {
      ...
      fs: {
        "/init": [
          "pkg install net_tools iputils dhcp",
          "iface eth0 wait link",
          "sleep 1",
          "dhclient eth0",
          "tun ipip add tun0 local 10.0.0.10 remote 10.0.0.11",
          "iface tun0 up",
          "iface tun0 add 10.10.0.1/24",
          "route add 10.10.0.0/24 dev tun0",
        ].join("\n"),
    ...
    },
    {
      ...
      fs: {
        "/init": [
          ...
          "sleep 2",
          "dhclient eth0",
          "tun ipip add tun0 local 10.0.0.11 remote 10.0.0.10",
          "iface tun0 up",
          "iface tun0 add 10.10.0.2/24",
          "route add 10.10.0.0/24 dev tun0",
        ].join("\n"),
        "/etc/hosts": "127.0.0.1 localhost",
      },
